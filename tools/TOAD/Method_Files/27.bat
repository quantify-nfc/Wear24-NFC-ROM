	...
	if exist Your_Files\%%a\*.* (
    	for /f %%b in ('dir /ad/b/on Your_Files\%%a') do (
		    if exist Your_Files\%%a\%%b\oat\%TOAD_ABI%\%%b.vdex (
				...
				rem This is the line that will convert our VDEX file into one or more DEX files
        		if not exist Tool_Files\*.dex (
            		set /a TOAD_DeodexErrors+=1
				) else (
                    rename Tool_Files\%%b_classes.dex classes.dex
			        if exist Tool_Files\%%b_classes*.dex (
       				    set temp=2
			            for /f %%c in ('dir /b/on Tool_Files\%%b_classes*.dex') do (
    	...
    	for /f %%b in ('dir /b/on Your_Files\%%a\oat\%TOAD_ABI%\*.vdex') do (
		    if exist Your_Files\%%a\%%~nb.jar (
        		...
        		if not exist Tool_Files\*.dex (
                    ...
                    rename Tool_Files\%%~nb_classes.dex classes.dex
			        if exist Tool_Files\%%~nb_classes*.dex (
       				    set temp=2
			            for /f %%c in ('dir /b/on Tool_Files\%%b_classes*.dex') do (
       					    if exist Tool_Files\%%c (
							    rename Tool_Files\%%c classes!temp!.dex
						...
						7za a -y -mx0 "%TOAD_StartDirectory%\TOAD_deodex_%TOAD_day%%TOAD_month%%TOAD_year%-%TOAD_hour%%TOAD_min%.zip" system >>"%TOAD_StartDirectory%\LOG.txt" 2>&1
        		        cd ..
					    echo.ui_print(" Deleting VDEX/ODEX files for '%%b'.."^);>>Tool_Files\META-INF\com\google\android\updater-script
...
if %TOAD_IncludeFramework%==yes (
    ...
    for /f %%a in ('java -Xmx1024m -jar Tool_Files\baksmali.jar list dex Your_Files\framework\%TOAD_ABI%\boot.oat') do (
	    rem The above line lists all the files that have their DEX files inside "boot.oat". We can't process them with baksmali.jar yet though.
		set Filename=%%~nxa
		if exist Your_Files\framework\!Filename! (
			...
			rem This is the line that will convert our VDEX file into one or more DEX files
       		if not exist Tool_Files\*.dex (
                ...
                rename Tool_Files\*_classes.dex classes.dex
    	        if exist Tool_Files\*_classes*.dex (
   				    set temp=2
		            for /f %%c in ('dir /b/on Tool_Files\*_classes*.dex') do (
   					    if exist Tool_Files\%%c (
						    rename Tool_Files\%%c classes!temp!.dex
						)
				        set /a temp+=1
	...
	:ProcessVDEXFiles_BOOT_Files_BOOT-dotVDEX
   	for /f %%a in ('dir /b/on Your_Files\framework\%TOAD_ABI%\boot-*.vdex') do (
	    ...
	    if exist Your_Files\framework\!Filename!.jar (
			...
			rem This is the line that will convert our VDEX file into one or more dDEX files
       		if not exist Tool_Files\*.dex (
           		set /a TOAD_DeodexErrors+=1
			) else (
                rename Tool_Files\%%~na_classes.dex classes.dex
		        if exist Tool_Files\%%~na_classes*.dex (
   				    set temp=2
		            for /f %%b in ('dir /b/on Tool_Files\%%~na_classes*.dex') do (
   					    if exist Tool_Files\%%b (rename Tool_Files\%%b classes!temp!.dex)
				        set /a temp+=1
			        )
		        )
		        if exist Tool_Files\*.tmp (
...
rem All the odexed JAR files inside "framework\%TOAD_ABI%\boot.oat" and
rem all the odexed 'boot-*' JAR files in "framework" should now be deodexed inside the ZIP file.
rem Any JAR file that didn't deodex has been left untouched.
rem If all the files could be deodexed, the ZIP file will delete the "%TOAD_ABI%" folder.
...
if %TOAD_Extension% == apk (
    ...
    rem This is the line that will try to extract the code from the VDEX file and create one or more DEX files. Vdexextractor.exe is a Linux program and uses "/" for the folders, NOT "\".
    if %TOAD_Address:~0,6%==vendor (
        for /f "tokens=1-4 delims=/ " %%a in ("!TOAD_Address!") do (
		    set TOAD_Address=%%a\%%b
		)
	...
	if not exist Tool_Files\*.dex (
        ...
        rename Tool_Files\%TOAD_Filename%_classes.dex classes.dex
        if exist Tool_Files\%TOAD_Filename%_classes*.dex (
		    set temp=2
            for /f %%a in ('dir /b/on Tool_Files\%TOAD_Filename%_classes*.dex') do (
...
rem We should now have a collection of fully deodexed APK and JAR files inside a flashable ZIP.
rem Or we should have a single deodexed file in the root folder, if the user selected that option.
exit /b
