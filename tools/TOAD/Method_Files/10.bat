				...
				for /f %%c in ('java -Xmx1024m -jar Tool_Files\baksmali.jar list dex Your_Files\%%a\%%~nb.odex') DO (
                    if !temp!==!TOAD_DeodexErrors! (
						...
						if %errorlevel%==0 (
						    ...
						    set /a temp2+=1
							if !temp2!==1 (
							    java -Xmx1024m -jar Tool_Files\smali.jar assemble -a 10 Tool_Files\out -o Tool_Files\classes.dex >>log.txt 2>&1
								if %errorlevel%==1 (
							    ...
							    java -Xmx1024m -jar Tool_Files\smali.jar assemble -a 10 Tool_Files\out -o Tool_Files\classes!temp2!.dex >>log.txt 2>&1
								if %errorlevel%==1 (
								    rem %errorlevel% will be 1 if smali.jar wasn't able to convert the SMALI files to a DEX file
									set /a TOAD_DeodexErrors+=1
									echo.>>log.txt
